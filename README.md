...
│   ├── position_store.py              # save/load live carried-position state (cost basis) across restarts; read at startup only when FLATTEN_ON_START=False — decoration, never touches trade logic
...
│   ├── __init__.py
│   └── order_executor.py             # OrderExecutor — LIMIT GTC (BUY); SELL is MARKET when urgent (stop-loss) else LIMIT GTC; 10-second stale BUY/SELL cancel; balance refresh
│
...
| **WebSocket session**  | `DEFAULT_SESSION_MINUTES`   | `150` min                                                | Default session length (fixed — no startup prompt)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
```
Binance REST API (production,      Binance WebSocket (production)
 keyless market_data_client)          │  diff_book_depth (100 ms)
    │  depth() snapshot                │  — same exchange as the snapshots:
    │  (seed + gap-recovery resync)    │    update IDs must be one sequence
    ▼                                  ▼
┌──────────────────┐       ┌──────────────────────┐
...
5. `AnalysisEngine` delegates order placement to `OrderExecutor`.
   - **Trigger** — when `_select_best_opportunity()` returns a non-`None` 8-element tuple `(level_idx, score, delta, total_depth, obi, micro_price, bq, aq)`, the engine calls `executor.execute("BUY", best_buy)` or `executor.execute("SELL", best_sell)`.
   - **Dispatch** — `OrderExecutor` validates the strategy, checks balances under `thread_balance_lock`, computes quantity (`aq` for BUY, `bq` for SELL), and sends the order via its own `SpotWebsocketAPIClient`.
   - **Order type** — a **LIMIT GTC** BUY; a SELL is **MARKET** for the stop-loss exit (`urgent=True`; immediate close, immune to a stale `local_book`) or a resting **LIMIT GTC** at `micro_price` for a planned mean-reversion exit (symmetric to the BUY).
   - **Resting-SELL resolution** — a resting LIMIT SELL may not fill, so after a 10 s timeout `cancel_stale_sell()` resolves it exactly like the BUY: cancelled-and-retried if still open, or treated as closed if it already filled.
   - **Guard ordering** — the position guard and stop-loss anchor stay armed until the exit actually fills, so a BUY can never stack behind an unfilled SELL.
   - **BUY sizing** — BUY orders are capped at `usdt / (micro_price × (1 + BACKTEST_FEE_RATE))`; dividing by price × (1 + fee) reserves the taker fee so the total debit never exceeds the available USDT balance.
   - **Balance refresh** — after every successful order, `_refresh_balance_rest()` re-fetches free balances from Binance REST to keep `state.balance_status` accurate in REST-fallback mode (where no `outboundAccountPosition` push events arrive).
   - **Stale-BUY cancel** — GTC BUY orders are automatically cancelled after 10 seconds via `cancel_stale_buy()`, freeing the locked USDT so the strategy can re-enter on the next signal.
   - **Async response** — the WS order response arrives asynchronously in `handle_order_response`.
...
7. **Real-time balance tracking (no listenKey)** — `OrderExecutor` owns the single `SpotWebsocketAPIClient` connection (`wss://ws-api.testnet.binance.vision/ws-api/v3`).
   - **Logon** — on socket open (`on_open` callback) it sends a signed `session.logon` frame. Because `on_open` can fire *during* the client constructor (before `ws_api_client` is assigned, silently swallowing that first attempt), `__init__` re-sends the logon after construction when `_logon_id` is still `None`.
   - **Subscribe** — on success it immediately sends `userDataStream.subscribe`.
   - **Push events** — once confirmed, Binance pushes `outboundAccountPosition` events on the **same** connection whenever a balance changes (e.g. after an order fill).
   - **Routing** — `handle_order_response` routes these push events (frames with no `"id"` field) to `_handle_balance_update`, which writes to `state.balance_status` under `thread_balance_lock`.
   - **Fallback** — if the testnet doesn't support `session.logon` or `userDataStream.subscribe`, the executor falls back silently to the REST snapshot taken at session startup.
8. **HMM regime filter** — `websocket_main.py` instantiates `RegimeDirector` and calls `get_klines_data()` → `select_hmm_model()` → `assign_regime_labels()` **before** the analysis threads start, so `regime_label` and `regime_confidence` are never `None` on the first `low_latency_analysis` iteration.
   - **Two-speed update** — every `historical_analysis` iteration runs a cheap Viterbi prediction (`predict_current_regime()`) on most iterations, and a full model re-fit (`select_hmm_model()`) every `HMM_REFIT_INTERVAL` (300 s, i.e. every 5th iteration).
   - **Scaling (no leakage)** — features are **z-score scaled** via `StandardScaler` before every `fit()` / `predict()` / `predict_proba()` call. The scaler is fitted on the first `train_end` rows only (in-sample), where `train_end = max(2, int(n_rows × 2/3))`, and applied to the full window (out-of-sample). At the default 120-row window `train_end = 80` (~⅔ in-sample, ~⅓ out-of-sample); shorter windows scale proportionally.
   - **Labelling** — `assign_regime_labels()` runs inside `_regime_lock` on every iteration.
   - **Gates** — `low_latency_analysis` reads both `regime_label` and `regime_confidence` under `_regime_lock` and applies two sequential gates before any order:
     - **Confidence gate** — if `regime_confidence < HMM_MIN_CONFIDENCE` (0.60), both BUY and SELL are skipped regardless of label (the model is uncertain).
     - **Direction gate** — BUY orders are suppressed in `"trending_down"` or `"high_volatility"` regimes; SELL orders are suppressed in `"trending_up"` or `"high_volatility"` regimes.
...
The **default of 150 minutes** is chosen deliberately:

| Metric                                          | Value at 150 min                                                                                 |
|-------------------------------------------------|--------------------------------------------------------------------------------------------------|
| Low-latency iterations (`low_latency_analysis`) | $150 \times 60 / 1 = \mathbf{9{,}000}$                                                           |
| Historical iterations (`historical_analysis`)   | $150 \times 60 / 60 = \mathbf{150}$                                                              |
| Order book snapshots in history                 | up to $150 \times 60 \times 10 = \mathbf{90{,}000}$ ticks (capped at `maxlen=3000` ≈ last 5 min) |
...
> **Thread Timeline** — For a step-by-step t=0s … t=150 min walkthrough of both threads, see [SYSTEM_ARCHITECTURE.md](SYSTEM_ARCHITECTURE.md).
...
6. Session duration fixed at `DEFAULT_SESSION_MINUTES` (150 min) — no user prompt.
...
8. Instantiate `OrderBookState`, `MessageHandler`, `OrderExecutor`, and `AnalysisEngine` (with `regime_director` injected), wiring the shared state.
    - `OrderExecutor` creates its own `SpotWebsocketAPIClient` internally (connected to `wss://ws-api.testnet.binance.vision/ws-api/v3`) with `on_open=self._on_ws_open` and `on_message=self.handle_order_response`.
    - On socket open it automatically sends `session.logon` → `userDataStream.subscribe` to enable real-time balance push events on the same connection.
    - Set `stop_event = threading.Event()`.
...
14. **Shutdown** — triggered after `session_seconds`, or early by a `KeyboardInterrupt` (Ctrl-C), which follows the same path:
    - Set `stop_event`, stop `ws_client`, and stop `executor` (closes the WS API + user-data connection).
    - Join both analysis threads (10 s low-latency, 15 s historical).
    - After threads exit, `executor.cancel_session_open_orders()` cancels every order this session placed that is still resting on the book — frees locked funds and prevents unattended fills after shutdown; foreign open orders on the shared testnet account are untouched.
    - Two reports are then printed:
    ...
    - **Balance report** — fetches final balances (free + locked, minus the foreign locked captured at session start — mirroring the equity chart's `locked_*_at_start` correction; free-only under-reports by the full locked amount) and `btc_end_price`, then prints a P&L decomposition:
      ...
      - **Buy & Hold benchmark** = `(end_price / start_price − 1) × 100` — what the whole starting equity would have returned if simply held as BTC, plus **Strategy vs B&H** (positive ⇒ the strategy beat holding BTC).  Benchmark only, *not* part of P&L; it mirrors the `bnh_index` line in the session P&L chart so the text report and the chart share one reference frame.
...
$$\texttt{regime\_confidence} = P(\text{state} = \text{current regime} \mid \text{observations})$$
...
State integers (0, 1, 2, …) carry no inherent meaning.  `assign_regime_labels()` assigns labels using a **rank-based** directional scheme **gated by an absolute-return threshold** (`REGIME_DIRECTIONAL_RETURN_THRESHOLD`, 0.0005 log-return per 5 m bar), plus threshold-based secondary rules:

**Directional labels** — at most one each (a best/worst state can earn *neither* label; see the return-threshold guard below):

A combined **direction score** is computed per state:

$$\texttt{direction\_score}_i = \operatorname{rank}(\texttt{return}_i) + \operatorname{rank}(\texttt{obi\_proxy}_i)$$

| Assignment        | Rule                                                                                                |
|-------------------|-----------------------------------------------------------------------------------------------------|
| `"trending_up"`   | State with the **highest** direction score **and** mean log-return > +`REGIME_DIRECTIONAL_RETURN_THRESHOLD` (0.0005) — genuinely bullish |
| `"trending_down"` | State with the **lowest** direction score **and** mean log-return < −`REGIME_DIRECTIONAL_RETURN_THRESHOLD` — genuinely bearish            |

`idxmax()`/`idxmin()` pick at most one best/worst state, so duplicate directional labels are structurally impossible. But rank alone is **not** sufficient: a best/worst state whose mean log-return sits **inside** ±`REGIME_DIRECTIONAL_RETURN_THRESHOLD` is non-directional and falls through to the secondary labels below (both BUY-eligible). Without this guard the lowest-ranked state would be tagged `trending_down` even in a flat or rising market, needlessly blocking the BUY side (observed 2026-07-14). The comparison is made in raw log-return-per-bar units, so the z-scored state means are un-standardised first.
...
| `backtest/sensitivity.py`                   | ✅ done | **Bayesian optimisation via Optuna TPE (default, 40 trials)**, OAT sweep (`--oat`, 8 runs), and deprecated full-grid (`--full-grid`) over `HMM_LOOKBACK_ROWS`, `VWAP_WINDOW`, and `VWAP_THRESHOLD_MULTIPLIER` on the **IS window** (270 days, ~77,760 5m / ~388,800 1m rows; `HMM_MAX_REGIMES` is pinned at 3 — a 2-state ceiling can never yield a `neutral` regime).  `fee_rate` fixed at `SENSITIVITY_FEE_RATE` (0.001).  `--lookback` flag overrides IS start.  `--flush-cache` clears the Parquet cache.  Writes `best_params.json` — loaded by `websocket_main.py` (live) and `runner.py` (backtest). Use Case B deferred.                                                                                                                                                                                                                                                                                                                                         |
...
| `vwap_window` | 5 | 60 | 5 |
| `vwap_threshold` | 0.002 | 0.003 | 0.0005 |

`fee_rate` is always fixed at `BACKTEST_FEE_RATE = 0.001` — it is a fixed
exchange cost, not a strategy knob.

The `vwap_threshold` range was restricted 2026-07-16 from `(0.001, 0.005)` to
`(0.002, 0.003)`: the `0.002` floor is the round-trip fee break-even
(`0.001` taker × 2) — below it Optuna converged on a fee-bleed degenerate — and
the `0.003` cap keeps the dead-zone tradeable (price rarely deviates 0.5 % from a
rolling intraday VWAP, so the old `0.005` max was untradeable live).
