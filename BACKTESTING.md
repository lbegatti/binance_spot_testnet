...
`simulate_pnl()` lets BUY legs **stack (pyramid)** via `open_strategy_qty` (BTC held exclusively by strategy signals, excluding `initial_btc`). Each leg spends at most `MAX_POSITION_PCT` (20 %) of the *available* USDT, and successive legs accumulate until invested exposure reaches `(1 − MIN_CASH_RESERVE_PCT)` = 65 % of mark-to-market equity — i.e. at least 35 % of the portfolio is always held as USDT. A BUY that would spend into the reserve is clamped; one fully blocked (cash already at/below the floor) is counted in `n_position_guard_skips` (returned in the `stats` dict). SELL always closes the full stacked position in one shot, guaranteeing the strategy returns to flat. The entry price is tracked as a VWAP across all open legs, which the adaptive stop-loss uses as its anchor.

> **Note (live/backtest parity):** this pyramiding model is now active on **both paths** (Phase 2 complete). The live path (`execution/order_executor.py` sizing clamp + `strategy/analysis.py` exposure gate) stacks BUY legs to the same `(1 − MIN_CASH_RESERVE_PCT)` = 65 % invested-exposure ceiling as `backtest/pnl.py`, additionally bounded by the live-only `MAX_PYRAMID_LEGS` safety cap. A full SELL or stop-loss resets the position to flat.
...
| **Position guard skips**       | `n_position_guard_skips`         | Count of BUY signals that passed all three gates but were suppressed because the **cash-reserve floor** was already reached (no spendable USDT above `MIN_CASH_RESERVE_PCT × equity`, i.e. the book is at its ~65 % invested cap). Stat key retained for backward-compat.                                                                                                                                          |
...
- ✅ **Step 4 — `backtest/pnl.py`** — Converts the signal DataFrame into
  ...
  exposure hits ``(1 − MIN_CASH_RESERVE_PCT)`` = 65 % of equity; SELL closes
